...
void PWMSetSpeedConsigne(float vitesseEnPourcents, char moteur) {
    if (moteur == MOTEUR_GAUCHE) {
        robotState.vitesseGaucheConsigne = -vitesseEnPourcents;      // Erreur du 05/03/2020
    } else {
        robotState.vitesseDroiteConsigne = vitesseEnPourcents; //-vitesse car moteurs dans des sens opposés
    }
}